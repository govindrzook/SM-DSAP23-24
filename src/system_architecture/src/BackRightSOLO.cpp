...
class BackRightSOLO : public rclcpp::Node
{
public:
  BackRightSOLO()
  : Node("back_right_solo")
  {
    back_right_torque_subscription = this->create_subscription<std_msgs::msg::Float64>(
      		"backRightTorque", 10, std::bind(&BackRightSOLO::back_right_torque_callback, this, _1));

    heartbeat_subscription = this->create_subscription<std_msgs::msg::UInt8>(
      		"heartbeat", 10, std::bind(&BackRightSOLO::heartbeat_callback, this, _1));

    estop_subscription = this->create_subscription<std_msgs::msg::UInt8>(
      		"estop", 10, std::bind(&BackRightSOLO::estop_callback, this, _1));

	  pub1_ = create_publisher<std_msgs::msg::Float64>("backRightSpeed", 10);

    pub_error = create_publisher<std_msgs::msg::String>("error", 10);

	  timer_ = create_wall_timer(std::chrono::seconds(1), std::bind(&BackRightSOLO::timer_callback, this));
    timer_heartbeat = create_wall_timer(std::chrono::seconds(1), std::bind(&BackRightSOLO::heartbeat_timer_callback, this));
...
private:
  ...
  void back_right_torque_callback(const std_msgs::msg::Float64 & msg) const{
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<BackRightSOLO>());
  rclcpp::shutdown();
  return 0;
}
